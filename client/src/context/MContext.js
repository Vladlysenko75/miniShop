    ...
    render() {
        return (
            <MContext.Provider value={
                {
                    state: this.state,
                    setCategory: (value) => this.setState({
                        category: value
                    }),
                    setCurrency: (value) => {
                        const currency = JSON.parse(localStorage.getItem('currency')) || {};
                    ...
                    addProductToCart: (event, prices, id) => {
                        ...
                        if (alreadyAddedProduct) {
                            ...
                            if (event) {
                                cart.push({id: id + selectedAttributes.join('-'), queryId: id, quantity: 1, prices: prices, attributes: selectedAttributes});
                                localStorage.setItem('cart', JSON.stringify(cart));
                                lastAddedProduct.time = Date.now();
                                localStorage.setItem('lastAddedProduct', JSON.stringify(lastAddedProduct));

                                cartProducts.push({id: id + selectedAttributes.join('-'), queryId: id, attributes: selectedAttributes, quantity: 1, prices: prices});
                                this.setState(prev => ({
                                    productsQuantity: prev.productsQuantity + 1,
                                    cartProducts,
                                }));
                    ...
                    deleteProduct: (id, quantity) => {
                        ...
                        this.setState(prev => ({
                            productsQuantity: prev.productsQuantity - quantity,
                            cartProducts: filteredProducts,
                        }));

                        this.countTotalPrice(filteredProducts);
                    },
                }}>
                {this.props.children}
            </MContext.Provider>)
